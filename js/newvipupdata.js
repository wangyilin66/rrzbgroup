...
function updateinfo(idinfo){
    ...
    base.commonAjax('clav/member/getClavMemberList', data, function (data) {
        ...
        $('#sr').val(data.data[0].birthday);
        $('#place').val(data.data[0].postalAddress);
        $('#szd').val(data.data[0].location);
        $('#placenum').val(data.data[0].postcode);
        $('#bgtec').val(data.data[0].officePhone);
